...
flag= fread(unlist(input[grepl('flag_list.txt', input)]))

if (grepl('harvest', input[1])){

flag= rename(flag, coreLMM= coreOK, phenoOK= phenotypesOK)

moms= unlist(input[grepl('runs_mfr_maternal.txt', input)])
dads= unlist(input[grepl('runs_mfr_paternal.txt', input)])
fets= unlist(input[grepl('runs_mfr_fetal.txt', input)])
moms= read.table(moms, h=T, sep= '\t')
dads= read.table(dads, h=T, sep= '\t')
fets= read.table(fets, h=T, sep= '\t')
q1= unlist(input[grepl('q1_v9.txt', input)])
q1= fread(q1)

fmoms= unlist(input[grepl('/results/het/maternal', input)])
fmoms= fread(fmoms)

fdads= unlist(input[grepl('/results/het/paternal', input)])
fdads= fread(fdads)

ffets= unlist(input[grepl('/results/het/fetal', input)])
ffets= fread(ffets)
...
m= unlist(input[grepl('maternal.hom', input)])
m= fread(m)

ms= m %>% group_by(CHR) %>% summarize(n= n(), KB= median(KB, na.rm=T))

d= unlist(input[grepl('paternal.hom', input)])
d= fread(d)

ds= d %>% group_by(CHR) %>% summarize(n= n(), KB= median(KB, na.rm=T))

fe= unlist(input[grepl('fetal.hom', input)])
fe= fread(fe)
...
res_m= unlist(input[grep('results/maps_lm/maternal/', input)])
res_m= fread(res_m)

res_f= unlist(input[grep('results/maps_lm/fetal/', input)])
res_f= fread(res_f)
...
fets= fread(fets)
q1= unlist(input[grep('q1_v9.txt', input)])
q1= fread(q1)
...
q1$AA1124= as.integer(as.factor(q1$AA1124))
q1$AA1125= as.integer(as.factor(q1$AA1125))
q1$AA1128= as.integer(as.factor(q1$AA1128))
q1$AA1129= as.integer(as.factor(q1$AA1129))
...
q1$AA1315= as.integer(as.factor(q1$AA1315))
q1$AA1316= as.integer(as.factor(q1$AA1316))
...
crudelmm= lmph(Surv(SVLEN_UL_DG, spont)~ FKB, moms)
crudelmd= lmph(Surv(SVLEN_UL_DG, spont)~ FKB, dads)
crudelmf= lmph(Surv(SVLEN_UL_DG, spont)~ FKB, fets)

minadjlmm= lmph(Surv(SVLEN_UL_DG, spont)~ FKB + PC1 + PC2 + PC3 + PC4 + PC5 + PC6 + PC7 + PC8 + PC9 + PC10 + PARITY0, moms)
minadjlmd= lmph(Surv(SVLEN_UL_DG, spont)~ FKB + PC1 + PC2 + PC3 + PC4 + PC5 + PC6 + PC7 + PC8 + PC9 + PC10 + PARITY0, dads)
minadjlmf= lmph(Surv(SVLEN_UL_DG, spont)~ FKB + PC1 + PC2 + PC3 + PC4 + PC5 + PC6 + PC7 + PC8 + PC9 + PC10 + PARITY0, fets)

fulllmm= lmph(Surv(SVLEN_UL_DG, spont)~ FKB + PC1 + PC2 + PC3 + PC4 + PC5 + PC6 + PC7 + PC8 + PC9 + PC10 + PARITY0 + edu, moms_c)
fulllmd= lmph(Surv(SVLEN_UL_DG, spont)~ FKB + PC1 + PC2 + PC3 + PC4 + PC5 + PC6 + PC7 + PC8 + PC9 + PC10 + PARITY0 + edu, dads_c)
fulllmf= lmph(Surv(SVLEN_UL_DG, spont)~ FKB + PC1 + PC2 + PC3 + PC4 + PC5 + PC6 + PC7 + PC8 + PC9 + PC10 + PARITY0 + edu, fets_c)

x= summary(crudelmm)$conf.int[1, c(1,3,4)]
x1= summary(minadjlmm)$conf.int[1, c(1,3,4)]
x2= summary(fulllmm)$conf.int[1, c(1,3,4)]
...
x= summary(crudelmd)$conf.int[1, c(1,3,4)]
x1= summary(minadjlmd)$conf.int[1, c(1,3,4)]
x2= summary(fulllmd)$conf.int[1, c(1,3,4)]
...
x= summary(crudelmf)$conf.int[1, c(1,3,4)]
x1= summary(minadjlmf)$conf.int[1, c(1,3,4)]
x2= summary(fulllmf)$conf.int[1, c(1,3,4)]
...
lmm= survfit(Surv(SVLEN_UL_DG, spont)~ QKB, moms)

p= ggsurvplot(
  lmm,                     # survfit object with calculated statistics.
...
fulllmml= lmph(Surv(SVLEN_UL_DG, spont)~ scale(KB_long) + PC1 + PC2 + PC3 + PC4 + PC5 + PC6 + PC7 + PC8 + PC9 + PC10 + PARITY0, moms)
fulllmdl= lmph(Surv(SVLEN_UL_DG, spont)~ scale(KB_long) + PC1 + PC2 + PC3 + PC4 + PC5 + PC6 + PC7 + PC8 + PC9 + PC10 + PARITY0, dads)
fulllmfl= lmph(Surv(SVLEN_UL_DG, spont)~ scale(KB_long) + PC1 + PC2 + PC3 + PC4 + PC5 + PC6 + PC7 + PC8 + PC9 + PC10 + PARITY0, fets)

x= summary(fulllmml)$conf.int[1, c(1,3,4)]
x1= summary(fulllmdl)$conf.int[1, c(1,3,4)]
x2= summary(fulllmfl)$conf.int[1, c(1,3,4)]
...
fulllmms= lmph(Surv(SVLEN_UL_DG, spont)~ scale(KB_short) + PC1 + PC2 + PC3 + PC4 + PC5 + PC6 + PC7 + PC8 + PC9 + PC10 + PARITY0, moms)
fulllmds= lmph(Surv(SVLEN_UL_DG, spont)~ scale(KB_short) + PC1 + PC2 + PC3 + PC4 + PC5 + PC6 + PC7 + PC8 + PC9 + PC10 + PARITY0, dads)
fulllmfs= lmph(Surv(SVLEN_UL_DG, spont)~ scale(KB_short) + PC1 + PC2 + PC3 + PC4 + PC5 + PC6 + PC7 + PC8 + PC9 + PC10 + PARITY0, fets)

x= summary(fulllmms)$conf.int[1, c(1,3,4)]
x1= summary(fulllmds)$conf.int[1, c(1,3,4)]
x2= summary(fulllmfs)$conf.int[1, c(1,3,4)]
...
freqm= freqm %>% separate(pos, into= c('pos1', 'pos2'), sep= ':')
freqm= mutate(freqm, chr= as.numeric(chr), pos1= as.numeric(pos1), pos2= as.numeric(pos2))
freqm$pos= (freqm$pos1 + freqm$pos2) / 2
...
#    geom_hline(yintercept = -log10(1*10^-5), size= 0.5, color= '#253494', alpha= 0.7) +
    geom_hline(yintercept = -log10(0.05/ nrow(don)), size= 0.5, color= 'gold', alpha= 0.7)
...
res_m= res_m %>% separate(id, c('chr', 'pos1','pos2'), ':')
res_m= res_m %>% mutate(chr= as.numeric(chr), pos1= as.numeric(pos1), 
pos2= as.numeric(pos2))
res_m$pos= (res_m$pos1 + res_m$pos2) / 2
...
names(res_f)= c('id','N', 'beta', 'sd', 'pvalue', 'schoenfeld', 'p_scheonfeld') 

freqf= freqf %>% separate(pos, into= c('pos1', 'pos2'), ':')
freqf= mutate(freqf, pos1= as.numeric(pos1), pos2= as.numeric(pos2))
freqf$pos= (freqf$pos1 + freqf$pos2) / 2


res_f= res_f %>% separate(id, c('chr', 'pos1','pos2'), ':')
res_f= res_f %>% mutate(chr= as.numeric(chr), pos1= as.numeric(pos1),
pos2= as.numeric(pos2))
res_f$pos= (res_f$pos1 + res_f$pos2) / 2
...
crudelmm= lmph(Surv(SVLEN_UL_DG, spont)~ none_F, moms)
crudelmd= lmph(Surv(SVLEN_UL_DG, spont)~ none_F, dads)
crudelmf= lmph(Surv(SVLEN_UL_DG, spont)~ none_F, fets)

minadjlmm= lmph(Surv(SVLEN_UL_DG, spont)~ none_F + PC1 + PC2 + PC3 + PC4 + PC5 + PC6 + PC7 + PC8 + PC9 + PC10 + PARITY0, moms)
minadjlmd= lmph(Surv(SVLEN_UL_DG, spont)~ none_F + PC1 + PC2 + PC3 + PC4 + PC5 + PC6 + PC7 + PC8 + PC9 + PC10 + PARITY0, dads)
minadjlmf= lmph(Surv(SVLEN_UL_DG, spont)~ none_F + PC1 + PC2 + PC3 + PC4 + PC5 + PC6 + PC7 + PC8 + PC9 + PC10 + PARITY0, fets)

fulllmm= lmph(Surv(SVLEN_UL_DG, spont)~ none_F + PC1 + PC2 + PC3 + PC4 + PC5 + PC6 + PC7 + PC8 + PC9 + PC10 + PARITY0 + edu, moms_c)
fulllmd= lmph(Surv(SVLEN_UL_DG, spont)~ none_F + PC1 + PC2 + PC3 + PC4 + PC5 + PC6 + PC7 + PC8 + PC9 + PC10 + PARITY0 + edu, dads_c)
fulllmf= lmph(Surv(SVLEN_UL_DG, spont)~ none_F + PC1 + PC2 + PC3 + PC4 + PC5 + PC6 + PC7 + PC8 + PC9 + PC10 + PARITY0 + edu, fets_c)
...
fulllmm= lmph(Surv(SVLEN_UL_DG, spont)~ none_F + PC1 + PC2 + PC3 + PC4 + PC5 + PC6 + PC7 + PC8 + PC9 + PC10 + PARITY0 + edu, moms_c)
fulllmm1= lmph(Surv(SVLEN_UL_DG, spont)~ soft_F + PC1 + PC2 + PC3 + PC4 + PC5 + PC6 + PC7 + PC8 + PC9 + PC10 + PARITY0 + edu, moms_c)
fulllmm2= lmph(Surv(SVLEN_UL_DG, spont)~ moderate_F + PC1 + PC2 + PC3 + PC4 + PC5 + PC6 + PC7 + PC8 + PC9 + PC10 + PARITY0 + edu, moms_c)
fulllmm3= lmph(Surv(SVLEN_UL_DG, spont)~ hard_F + PC1 + PC2 + PC3 + PC4 + PC5 + PC6 + PC7 + PC8 + PC9 + PC10 + PARITY0 + edu, moms_c)

x= summary(fulllmm)$conf.int[1, c(1,3,4)]
x1= summary(fulllmm1)$conf.int[1, c(1,3,4)]
x2= summary(fulllmm2)$conf.int[1, c(1,3,4)]
x3= summary(fulllmm3)$conf.int[1, c(1,3,4)]
...
fulllmd= lmph(Surv(SVLEN_UL_DG, spont)~ none_F + PC1 + PC2 + PC3 + PC4 + PC5 + PC6 + PC7 + PC8 + PC9 + PC10 + PARITY0 + edu, dads_c)
fulllmd1= lmph(Surv(SVLEN_UL_DG, spont)~ soft_F + PC1 + PC2 + PC3 + PC4 + PC5 + PC6 + PC7 + PC8 + PC9 + PC10 + PARITY0 + edu, dads_c)
fulllmd2= lmph(Surv(SVLEN_UL_DG, spont)~ moderate_F + PC1 + PC2 + PC3 + PC4 + PC5 + PC6 + PC7 + PC8 + PC9 + PC10 + PARITY0 + edu, dads_c)
fulllmd3= lmph(Surv(SVLEN_UL_DG, spont)~ hard_F + PC1 + PC2 + PC3 + PC4 + PC5 + PC6 + PC7 + PC8 + PC9 + PC10 + PARITY0 + edu, dads_c)

x= summary(fulllmd)$conf.int[1, c(1,3,4)]
x1= summary(fulllmd1)$conf.int[1, c(1,3,4)]
x2= summary(fulllmd2)$conf.int[1, c(1,3,4)]
x3= summary(fulllmd3)$conf.int[1, c(1,3,4)]
...
fulllmf= lmph(Surv(SVLEN_UL_DG, spont)~ none_F + PC1 + PC2 + PC3 + PC4 + PC5 + PC6 + PC7 + PC8 + PC9 + PC10 + PARITY0 + edu, fets_c)
fulllmf1= lmph(Surv(SVLEN_UL_DG, spont)~ soft_F + PC1 + PC2 + PC3 + PC4 + PC5 + PC6 + PC7 + PC8 + PC9 + PC10 + PARITY0 + edu, fets_c)
fulllmf2= lmph(Surv(SVLEN_UL_DG, spont)~ moderate_F + PC1 + PC2 + PC3 + PC4 + PC5 + PC6 + PC7 + PC8 + PC9 + PC10 + PARITY0 + edu, fets_c)
fulllmf3= lmph(Surv(SVLEN_UL_DG, spont)~ hard_F + PC1 + PC2 + PC3 + PC4 + PC5 + PC6 + PC7 + PC8 + PC9 + PC10 + PARITY0 + edu, fets_c)

x= summary(fulllmf)$conf.int[1, c(1,3,4)]
x1= summary(fulllmf1)$conf.int[1, c(1,3,4)]
x2= summary(fulllmf2)$conf.int[1, c(1,3,4)]
x3= summary(fulllmf3)$conf.int[1, c(1,3,4)]
