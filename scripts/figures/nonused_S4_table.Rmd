...
colnames(x)= c('chr', 'pos', 'snpID', 'REF', 'ALT', 'INFO', 'AF')
...
df= select(df, variant, ALT, REF)
df= df[!duplicated(df$variant), ]
...
kable(d, align= c('l', rep('c', 8)), row.names= F, caption = "S4 Table: Maternal genetic variants associated with time-to-spontaneous delivery under a recessive model.", booktabs = T) %>% kable_styling(latex_options= c('striped', 'scale_down')) %>% column_spec(1, bold = T, color = "black", background = "white") %>% footnote(general_title= '', general= c('chr, chromosome; pos, position; REF, reference allel; ALT, effect allele; sd, standard deviation.', 'Association results for accelerated failure time model on spontaneous delivery under a recessive model. Only genetic variants with genome-wide significance are shown. Effect allele refers to minor allele, and the estimate (and beta) expresses the effect of being homozygous for this allele. MAF was calculated using all sub-cohorts, and the estimate, beta and p-value were obtained after inverse-variance weighted meta-analysis of individual cohort beta and sd. Gene column shows the nearest protein coding gene, and Mim the OMIM number for that specific gene, if any.'), threeparttable= T)
