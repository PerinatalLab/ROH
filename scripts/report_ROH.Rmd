...
moms= input[grep(input, 'runs_mfr_maternal.txt')]
dads= input[grep(input, 'runs_mfr_paternal.txt')]
fets= input[grep(input, 'runs_mfr_fetal.txt')]
moms= read.table(moms, h=T, sep= '\t')
dads= read.table(dads, h=T, sep= '\t')
fets= read.table(fets, h=T, sep= '\t')
q1= input[grep(input, '_q1_v9.csv')]
...
m= input[grep(input, 'harvestm12_maternal.hom')]
m= fread(m)
m24= input[grep(input, 'harvestm24_maternal.hom')]
m24= fread(m24)

m= rbind(m, m24)
ms= m %>% group_by(CHR) %>% summarize(n= n(), KB= median(KB, na.rm=T))

d= input[grep(input, 'harvestm12_paternal.hom')]
d= fread(d)
d24= input[grep(input, 'harvestm24_paternal.hom')]
d24= fread(d24)

d= rbind(d, d24)
ds= d %>% group_by(CHR) %>% summarize(n= n(), KB= median(KB, na.rm=T))

fe= input[grep(input, 'harvestm12_fetal.hom')]
fe= fread(fe)
fe24= input[grep(input, 'harvestm24_fetal.hom')]
fe24= fread(fe24)
...
freqm= input[grep(input, 'runs/frequency/ROH_frequency_maternal')]
freqm= read.table(freqm, h=F, sep= '\t')
names(freqm)= c('chr','pos','freqm')

freqd= input[grep(input, 'runs/frequency/ROH_frequency_paternal')]
freqd= read.table(freqd, h=F, sep= '\t')
names(freqd)= c('chr','pos','freqd')

flist= input[grep(input, '/ROH/harvest/results/maps_cox/maternal/')]
res_m= lapply(flist, read.table)
res_m= do.call("rbind", res_m)
res_m= data.frame(res_m)

flist= input[grep(input, '/mnt/work/pol/ROH/harvest/results/maps_cox/fetal/')]
...
m= input[grep(input, 'rotterdam1_maternal.hom')]
m= fread(m)

ms= m %>% group_by(CHR) %>% summarize(n= n(), KB= median(KB, na.rm=T))
d= input[grep(input, 'rotterdam1_paternal.hom')]
d= fread(d)

ds= d %>% group_by(CHR) %>% summarize(n= n(), KB= median(KB, na.rm=T))

fe= input[grep(input, 'rotterdam1_fetal.hom')]
fe= fread(fe)
...
freq= input[grep(input, '/mnt/work/pol/ROH/rotterdam1/runs/frequency/ROH_frequency_maternal')]
freqm= read.table(freqm, h=F, sep= '\t')
names(freqm)= c('chr','pos','freqm')

freqd= input[grep(input, '/mnt/work/pol/ROH/rotterdam1/runs/frequency/ROH_frequency_paternal')]
freqd= read.table(freqd, h=F, sep= '\t')
names(freqf)= c('chr','pos','freqd')

freq= inner_join(freqm, freqf, by= c('chr','pos'))

flist_m= input[grep(input, '/mnt/work/pol/ROH/rotterdam1/results/maps_cox/maternal/')]
res_m= lapply(flist_m, read.table)
res_m= do.call("rbind", res_m)
res_m= data.frame(res_m)

flist_f= input[grep(input,'/mnt/work/pol/ROH/rotterdam1/results/maps_cox/fetal/')]
